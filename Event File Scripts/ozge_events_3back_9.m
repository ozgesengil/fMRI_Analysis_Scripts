...
% for fir analysis 2
% Events as onsets: episodes consisting of trials:1,2,3,4 (in accordance
%                   with the number of targets in that particular episode being 0,1,2,3)
    ...
    for i = 1:max(aa(:,2)) % one ep at a time
        ...
        for ii = 1:ep_id
            ...
            if mod(ii,trialnum) == 1 % if it is the first trial of the episode
                ...
                if sum(ep(:,4))==9 % if there is 0 target in the episode
                    ev(a,2) = 1;
                elseif sum(ep(:,4))==10 % if there is 1 target in the episode
                    ev(a,2) = 2;
                elseif sum(ep(:,4))==11 % if there is 2 targets in the episode
                    ev(a,2) = 3;    
                elseif sum(ep(:,4))==12 % if there is 3 targets in the episode
                    ev(a,2) = 4;
                end
