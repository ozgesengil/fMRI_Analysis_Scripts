...
% Duration of the event is also modelled

% Whether the response to the trial is correct is also modelled (5th
% column)
    % Fifth column: 0: incorrect response, episode
    %               1: correct response, episode
    %               2: interepisode interval, at the beginning of the first
    %               trial of an episode, after 'press a button to cont'
    %               screen, starting with second episode (the first episode
    %               doesnot have that)
    %               3: big rests
    ...
    for i = 1:max(aa(:,2)) % one ep at a time
        ...
        for ii = 1:ep_id
            ...
            if mod(ii,trialnum) == 1 % if it is the first trial of the episode
                ...
                ev(a,5) = ep(1,7); % whether there is a correct response in the episode, 1 if ocrrect, 0 if incorrect
            ...
            elseif mod(ii,trialnum)==0  % if it is the last trial of the episode
                ev(a,4) = ep(ii,5)+ep(ii,6)-ee; % Reaction moment-the onset of the first picture of the episode
                    ...
                    ev(a+1,5) = 2; % interepisode interval
                    c=a+1;
                    a=a+2;
                    end
