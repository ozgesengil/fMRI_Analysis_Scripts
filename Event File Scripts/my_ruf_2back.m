...
for ntot = 1:length(subs)
    ...
    for i = 1:max(aa(:,2)) % one ep at a time
        ...
        for ii = 1:ep_id
            ...
            if mod(ii,trialnum)==0  % inter-episode interval after the episode started (calculated from the first trial of each episode)
                ...
                ev(a+1,1) = ntot;
                ev(a+1,2) = 10; % inter-episode interval 1
                ev(a+1,3) = ep(1,10) - Exp.expstarttime;
                ev(a+1,4) = ep(1,5)- ep(1,10);
                ...
                if i ~= max(aa(:,2))
                    ev(a,1) = ntot; % inter-episode interval 2
                    ev(a,2) = 11;
    ...
    if aa(size(aa,1),1)==100 % if there is a rest data
        ev(a,1) = ntot;
        ev(a,2) = 12;
        ev(a,3) = aa(size(aa,1),5)-Exp.expstarttime; % beginning rest onset
