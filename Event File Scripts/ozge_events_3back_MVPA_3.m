% Event File Code, MVPA 3

   %%% MVPA3 WITH ONLY 2-BACK PARTICIPANTS %%%
% for subs 02,03 and 04 --- events are not present in the given positions (at
% least 1 event) - they will be excluded from this analysis
% for subs 23-26 --- never targets didnot occur in the first 2 or 3
% positions (they occurred in the middle positions), so they are excluded
% from this analysis as well.



% Events: 
%         1st and 2nd trials are modeled as onset (NEVER target pictures
...
for ntot = 1:length(subs)
    ...
    for i = 1:max(aa(:,2)) % one ep at a time
        ...
        for ii = 1:ep_id
             ...
             if ii==1 || ii==2 || ii==3 % if it is trial 1 or 2
