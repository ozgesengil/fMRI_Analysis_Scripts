% will be used for participants sub01, sub02, sub03, sub04, and sub23 onwards...
...
addpath 'C:\Users\HCF-Lab\Documents\MATLAB\spm12\spm_allowmultiplebfs';

rootdir = 'E:\OZGE_STUDY2';        % top level study directory
statsdir ='E:\OZGE_STUDY2\statsfir1';           % directory for analysis results
evdir = 'E:\OZGE_STUDY2\eventsfir1';     % directory with event onset files
...
subs = {'sub01','sub02','sub03','sub04','sub23','sub24','sub25','sub26'};
subs = char(subs);
...
myxBF(1).length = 90; % length in seconds
myxBF(1).order = 45; % order of basis set

% myxBF(2).name = 'Finite Impulse Response';
% myxBF(2).length = 36; % length in seconds
% myxBF(2).order = 18; % order of basis set
% 
% myxBF(3).name = 'Finite Impulse Response';
% myxBF(3).length = 36; % length in seconds
% myxBF(3).order = 18; % order of basis set
% 
% myxBF(4).name = 'Finite Impulse Response';
% myxBF(4).length = 36; % length in seconds
% myxBF(4).order = 18; % order of basis set

myxBF(2).name = 'Finite Impulse Response';
myxBF(2).length = 16; % length in seconds % average lenght of iei
myxBF(2).order = 8; % order of basis set

myxBF(3).name = 'Finite Impulse Response';
myxBF(3).length = 20; % length in seconds % average lenght of rest
myxBF(3).order = 10; % order of basis set
...
bfnum=[1 2 3]; % hrf for first two conditions, FIR for remainder
...
for sub = 1:ntot               % loop through subjects
    ...
    nsess = length(Sess);
    bfnum=[1 1 1 1 2 3]; % hrf for first two conditions, FIR for remainder

    for m=1:nsess % 3 sessions

        for n=1:3 % 3 conditions
