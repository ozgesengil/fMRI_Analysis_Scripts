...
firstlevel_dir = 'G:\OZGE_STUDY2\statsMVPA1';
cd(firstlevel_dir);
% subject_folders = dir;
% subject_folders = {subject_folders(3:end-1).name};
subject_folders =  {'sub02','sub03','sub04','sub06','sub07','sub08','sub09','sub10','sub11','sub12','sub13','sub14','sub15','sub16','sub17','sub18','sub19','sub20','sub21','sub22','sub23','sub24','sub25','sub26','sub27','sub28', 'sub30','sub31','sub32','sub33'};
sessnums=[3,3,3,4,2,4,4,3,4,3,4,4,4,4,4,4,4,4,4,4,3,3,4,4,3,3,3,3,3,3,3];
...
for sub = 1:numel(subject_folders)
    ...
    cond_type = {'at1', 'at2'};%{'s\d_\d', 'n\d_\d'};
    for ntype = 1:2
...
%         'rL_ESV.nii', 'rR_ESV.nii'};
    ...
    for rois = 1:numel(roi_list)
        ...
        % if participant has 3 sessions
      if sessnums(sub)==3
%         for i = 1:3
        train_ind = [type_all{1}(1:3), type_all{2}(1:3)];
        test_ind = [type_all{1}(4:6), type_all{2}(4:end)];
...
%         
      ...
      elseif sessnums(sub)==4 % if participant has 4 sesssions
%         for i = 1:3
