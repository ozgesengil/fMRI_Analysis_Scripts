...
filename='tl_epilepsy_bodilysens_sub07_run1'; % .mat file with behavioral data that will be saved
...
% DrawFormattedText(window,'Ã‡alÄ±ÅŸma baÅŸlamak Ã¼zere...','center','center', [0 0 0], [],1);
% Screen('Flip',window);
% 
% KbQueueStart();
% curTR=0;
...
for i=1:length(blocklist)
    ...
    aa=1;
            ...
            DrawFormattedText(window,'Devam Etmek için Tusa Basiniz','center','center',red);
            Screen('Flip',window);
            ...
            while buttonpress==0
                ...
                elseif pressed && keyCode(Key.red) % if a seizure started
                        ...
                        DrawFormattedText(window,'nöbet ekrani \n\n lutfen bekleyiniz','center','center',black);
                        Screen('Flip',window);
                        ...
                        while buttonpress==0 % waiting for the end of the seizure
                         ...
                         timeSecs = keyCode(find(keyCode));
                                ...
                                elseif pressed && keyCode(Key.green) % if seizure ended
                                    ...
                                    xx=2;
                                    buttonpress=1;
                                    DrawFormattedText(window,'dinlenin','center','center',black);
                                    Screen('Flip',window);
          ...
          if aa==2 % if just got out of a seizure block occured during a fixation, before moving on to the next block wait for a couple of seconds
              Screen('FillRect', window, white);
              Screen('DrawDots', window, [xCenter; yCenter], 15, [0 0 0], [], 2);
              Screen('Flip',window);
              aa=1;
    ...
    if blocklist(i)==1
        ...
        DrawFormattedText(window,'RAHIM hislerine ODAKLAN','center','center',black);
       
    elseif blocklist(i)==2
        Screen('FillRect', window, blue); 
        DrawFormattedText(window,'VÜCUT hislerine ODAKLAN','center','center',black);
       
    elseif blocklist(i)==3
        Screen('FillRect', window, green); 
        DrawFormattedText(window,'DINLEN','center','center',black,[]);

    elseif blocklist(i)==4
        Screen('FillRect', window, yellow); 
        DrawFormattedText(window,'NOBET hislerini HAYAL ET','center','center',black,[]);
 
    elseif blocklist(i)==5
        Screen('FillRect', window, purple); 
        DrawFormattedText(window,'vücutsal/dokunsal bir olayi HAYAL ET','center','center',black,[]);
    ...
    response_period_start=GetSecs;
        ...
        while GetSecs < response_period_start+response_period && xx==1 % while the block continues
            ...
            if pressed && keyCode(Key.red) % if green is pressed (the seizure start)
                ...
                Screen('FillRect', window, gray); 
                DrawFormattedText(window,'nöbet ekrani \n\n lütfen bekleyiniz','center','center',black);
                Screen('Flip',window);
                Exp.IctalSeizure(2,seizurecount)=i; % during which block it occured
                Exp.IctalSeizure(3,seizurecount)=blocklist(i); % during what type of a block it occured
