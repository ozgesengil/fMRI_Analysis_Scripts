...
% Here are the responding rows of output/performance matrices (in total it has 11):
    ...
    % 11. Reaction Time
    % 12. Onset of rest screen
    % 13. 'Press any key to cont.' screen onset
    % 14. 'Press any key to cont.' screen response timing
    % 15. 'Press any key to cont.' screen RT (10-9)
...
filename='practice_sub_elif_poyraz_ruleswitch_2'; % .mat file with behavioral data that will be saved
...
%in FMRI
DrawFormattedText(window,'Hosgeldiniz...','center','center', [0 0 0],[]);
Screen('Flip',window);
...
for i = 1:1
    ...
    DrawFormattedText(window, num2str(11-i), 'center', 'center',[0 0 0],1);
    Screen('Flip', window);
    WaitSecs(1);
       ...
       iBi= [1.5 2 2.5 3 3.5 4 5 6 7 7.5 8];
%% Experimental Loop
        ...
        for i = 1:numtrials
            ...
            Screen('FrameRect', window, curr_color, rectpos, lineWidth); % frame for the selected condition/color
                        ...
                        if bigfont_side==1 % the bigger font for the left side
                            ...
                            DrawFormattedText(window, stimuli1, xCenter-150, 'center', theColor,[]);
                        ...
                        elseif bigfont_side==2 % the bigger font for the right side
                            ...
                            DrawFormattedText(window, stimuli1, xCenter-150, 'center', theColor,[]);
                            Screen('TextSize', window, font_size(1));
                            DrawFormattedText(window, stimuli2, xCenter+150, 'center', theColor,[]); 
                        end
            ...
            while press==0
             ...
             if pressed && (keyCode(escapeKey) || keyCode(KeyEscape) || keyCode(Key.blue) || keyCode(Key.yellow)) % make sure it is not sensitive to TR pulses
              ...
              if curr_block_type==1 % if it is a rule repeat block (resp_font will determine the correct answer)
                ...
                elseif keyCode(Key.yellow) && resp_font==1 % if left is pressed and bigger font is on the left
                    respMatA(10, i) = 0; % correct
                    respMatA(11, i) = secs(1) - tStart;
                    press=1;
                    
                elseif keyCode(Key.blue) && resp_font==0 % if right is pressed and bigger font is on the right
                    respMatA(10, i) = 0; % correct
                    respMatA(11, i) = secs(1) - tStart;
                    press=1;
                ...
                if  curr_task==1 % green frame
                    ...
                    elseif keyCode(Key.yellow) && resp_font == 1 % bigger font on the left and left key 
                        respMatA(10, i) = 0; % correct
                        respMatA(11, i) = secs(1) - tStart;
                        press=1;
                    
                    elseif keyCode(Key.blue) && resp_font == 0 % bigger font on the right and right key 
                        respMatA(10, i) = 0; % correct
                        respMatA(11, i) = secs(1) - tStart;
                        press=1;
                ...
                elseif  curr_task==2 % blue frame
                    ...
                    elseif keyCode(Key.yellow) && resp_numvalue == 1 % bigger number the left and left key 
                        respMatA(10, i) = 0; % correct
                        respMatA(11, i) = secs(1) - tStart;
                        press=1;
                    
                    elseif keyCode(Key.blue) && resp_numvalue == 0 % bigger number on the right and right key 
                        respMatA(10, i) = 0; % correct
                        respMatA(11, i) = secs(1) - tStart;
                        press=1;
