function result = ciwithinnan(matrix,alpha,color1)
...
%data around 0.
new_matrix=matrix-(nanmean(matrix,2));
...
new_matrix=new_matrix+mean(nanmean(matrix,2));
...
normalized_mean = nanmean(new_matrix);
normalized_SD = nanstd(new_matrix);
...
CIdist_Lower_m = abs(nanmean(new_matrix)-CILower_m);
CIdist_Higher_m = abs(nanmean(new_matrix)-CIHigher_m);
 

[nbars, ngroups] = size(nanmean(new_matrix));

b=bar(nanmean(new_matrix));
...
errorbar(x', nanmean(new_matrix), CIdist_Lower_m, CIdist_Higher_m,'k','linestyle','none');
er.Color = [0 0 0];
b.FaceColor=color1;
